 ...
 *
 * @version $Id: WorkerSessionBean.java 6135 2015-07-14 11:01:58Z netmackan $
 */
@Stateless
public class WorkerSessionBean implements IWorkerSession.ILocal,
    ...
    private byte[] encryptMessage(AdminInfo adminInfo, int workerId, String message) throws InvalidWorkerIdException, NoSuchPaddingException, UnsupportedEncodingException, NoSuchAlgorithmException, IllegalBlockSizeException, org.cesecore.keys.token.CryptoTokenOfflineException, BadPaddingException, InvalidKeyException {
        ...
        try {
            encryptedMessage = signer.encryptMessage(alias, authCode, message, servicesImpl);
        } catch (CryptoTokenOfflineException e) {
            throw new IllegalArgumentException("CryptoTokenOfflineException: ", e);
        }
    ...
    @Override
    public Certificate generateSignerKeyAndGetCertificate(final int signerId, String keyAlgorithm, String keySpec, String alias, final char[] authCode)
            throws CryptoTokenOfflineException, InvalidWorkerIdException, IllegalArgumentException, KeyStoreException {
        return generateSignerKeyAndGetCertificate(new AdminInfo("TigerSigner", null, null), signerId, keyAlgorithm,
                keySpec, alias, authCode);
    }

    private Certificate generateSignerKeyAndGetCertificate(AdminInfo tigerSigner, int signerId, String keyAlgorithm, String keySpec, String alias, char[] authCode) throws InvalidWorkerIdException, CryptoTokenOfflineException, KeyStoreException {
        IWorker worker = workerManagerSession.getWorker(signerId, globalConfigurationSession);
        if (worker == null) {
            throw new InvalidWorkerIdException("Given SignerId " + signerId
                    + " doesn't exist");
        ...
        try {
            return signer.generateSignerKeyAndGetCertificate(keyAlgorithm, keySpec, alias, authCode, Collections.<String, Object>emptyMap(), servicesImpl);
        } catch (DuplicateAliasException ex) {
            throw new IllegalArgumentException("The specified alias already exists");
        } catch (NoSuchAlgorithmException ex) {
            if (LOG.isDebugEnabled()) {
    ...
    @Override
    public void uploadSignerCertFromCrytoWorkerCert(int signerId, int crytptoTokenID,
                                                    String scope) throws CertificateException, CryptoTokenOfflineException {
        uploadSignerCertFromCrytoWorkerCert(new AdminInfo("Tiger Signer", null, null), signerId, crytptoTokenID, scope);
    }

    public void uploadSignerCertFromCrytoWorkerCert(AdminInfo adminInfo, int signerID, int crytoTokenID, String scope) throws CryptoTokenOfflineException, CertificateException {
        WorkerConfig config = getWorkerConfig(signerID);

        final Certificate cryptoTokenCertificate = getSignerCertificate(crytoTokenID);

        (new ProcessableConfig(config)).setSignerCertificate((X509Certificate) cryptoTokenCertificate, scope);
        ...
        auditLogCertInstalled(adminInfo, signerID, new String(CertTools.getPEMFromCerts(Arrays.asList(cryptoTokenCertificate))), scopeGlobal ? "GLOBAL" : "NODE", scopeGlobal ? null : WorkerConfig.getNodeId());
    }
    ...
    public void uploadSignerCertChainFromCrytoWorkerCert(AdminInfo adminInfo, int signerID, int crytoTokenID, String scope) throws CryptoTokenOfflineException, CertificateException {
        WorkerConfig config = getWorkerConfig(signerID);

        final List<Certificate> cryptoTokenCertificates = getSignerCertificateChain(crytoTokenID);

        (new ProcessableConfig(config)).setSignerCertificateChain(cryptoTokenCertificates, scope);
        setWorkerConfig(adminInfo, signerID, config, null, null);
        final boolean scopeGlobal = GlobalConfiguration.SCOPE_GLOBAL.equalsIgnoreCase(scope);
        auditLogCertInstalled(adminInfo, signerID, new String(CertTools.getPEMFromCerts(cryptoTokenCertificates)), scopeGlobal ? "GLOBAL" : "NODE", scopeGlobal ? null : WorkerConfig.getNodeId());
    }
